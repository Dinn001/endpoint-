...
async function fetchHTML(url, timeout = 14000) {
  ...
  try {
    ...
    return {
      success: false,
      error: err.name === "AbortError" ? "Request timeout (>14s)" : err.message,
      ping: Date.now() - start
    };
  } finally {
    clearTimeout(id);
...
export default async function handler(req, res) {
  try {
    ...
    if (category === "ai" && name === "text-to-speech") {
      ...
      try {
        ...
        if (!r.success || !r.data?.status) {
          return res.status(502).json({ success: false, error: "Gagal generate suara", raw: r.raw || null });
        }
    ...
    // ===================================================
    // 📡 TOOLS — TRI CHECK (MENGGUNAKAN AMAN NYA KODE TEXT)
    // ===================================================
    if (category === "tools" && name === "tricek") {
      ...
      try {
        ...
        // 🔥 PAKAI AMAN: Ambil text-nya dulu agar tri.co.id kalau down tidak merusak JSON parser
        const text = await response.text();
        let result;
        try {
          result = JSON.parse(text);
        } catch {
          return res.status(502).json({ success: false, error: "Response server Tri bermasalah (Bukan JSON)", raw: text.slice(0, 150) });
        }
        ...
        return res.status(500).json({ success: false, error: "Server Tri Error" });
      }
    }
    ...
    if (category === "tools" && name === "remini") {
      ...
      try {
        ...
        if (!r.success || !r.data?.status) return res.status(502).json({ success: false, error: "Gagal enhance gambar" });
    ...
    if (category === "tools" && name === "cekxl") {
      ...
      const start = Date.now();
      try {
        const response = await fetch("https://apigw.kmsp-store.com/sidompul/v4/cek_kuota?" + new URLSearchParams({ msisdn: num, isJSON: "true" }), {
          method: "GET",
          headers: {
        ...
        try {
          ...
          return res.status(502).json({ success: false, error: "Response bukan JSON", raw: text.slice(0, 300) });
        }

        if (!json.status) {
          return res.status(400).json({ success: false, error: json?.data?.keteranganError || json?.message || "Gagal mengambil data" });
        }
